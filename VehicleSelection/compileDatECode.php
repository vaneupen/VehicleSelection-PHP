			...
			<?php
				...
				try {
					...
					$request = new SoapVar('<request>
								...
								<selectedOptions>'.$optEngine.'</selectedOptions><!-- Motor (Gruppe 1) -->
								<selectedOptions>'.$optCarBody.'</selectedOptions><!-- Karosserie (Gruppe 2) -->
								<selectedOptions>'.$optGearing.'</selectedOptions><!-- Getriebe (Gruppe 11) -->
								<selectedOptions>'.$optEquipLine.'</selectedOptions><!-- Ausstattungslinie (Gruppe 10) -->            
								</request>',XSD_ANYXML);

					$tag['compileDatECode'] = $request;
					...
					$objConversions = $wsdlDatei_eval2 -> __soapCall('compileDatECode', $getCardata);
