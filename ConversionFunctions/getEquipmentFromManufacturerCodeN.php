...
<body>
	...
	<section>
		<article>
		<h1>getEquipmentFromManufacturerCodeN</h1>
			<p>
			<?php
				...
				try {
					...
					$wsdlDatei_eval2 = new SoapClient($host."/DATECodeSelection/services/ConversionFunctionsService?wsdl",
										array('features' => SOAP_WAIT_ONE_WAY_CALLS, 'trace' => true,'exceptions' => 1,
										'cache_wsdl'=>'WSDL_CACHE_NONE','encoding' => 'UTF-8',
										'uri' => 'http://sphinx.dat.de/services/ConversionFunctionsService',
										'soap_version' => SOAP_1_1));
					...
					$request = new SoapVar('<request>
								...
								<manufacturerCodes>'.$manufacturerCodes.'</manufacturerCodes>
								</request>',XSD_ANYXML);

					$tag['getEquipmentFromManufacturerCodeN'] = $request;
					...
					$objConversions = $wsdlDatei_eval2 -> __soapCall('getEquipmentFromManufacturerCodeN', $getCardata);
